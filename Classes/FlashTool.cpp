...
Animate * FlashTool::readJsonSpriteSheet(std::string jsonFile, float delayPerUnit) {
    ...
    for (auto m = frames.MemberBegin(); m != frames.MemberEnd(); m++) {
        ...
        if (!sf) {
            sf = SpriteFrame::create(imgFileName, 
				Rect(frameProperties["x"].GetInt(), frameProperties["y"].GetInt(), 
					frameProperties["w"].GetInt(), frameProperties["h"].GetInt()), 
				m->value["rotated"].GetBool(), 
				Vec2(spriteSourceSize["x"].GetInt(), spriteSourceSize["y"].GetInt()), 
				Size(spriteSourceSize["w"].GetInt(), spriteSourceSize["h"].GetInt()));
